...
class UITableViewMovingViewController: BaseViewController {
...
extension UITableViewMovingViewController: UITableViewDataSource, UITableViewDelegate {
...
//        tableView.setEditing(false, animated: true)
    ...
    // 计算下标, 如果垮了section，dest row需要额外减1
    func getIndicesBySrcAndDest(_ src: IndexPath, _ dest: IndexPath) -> [Int]{
        let srcIdx: Int = src.section * numberPerSection + src.row
        var destIdx: Int = dest.section * numberPerSection + dest.row
        if src.section < dest.section {
            destIdx -= 1
        }
    ...
    func tableView(_ tableView: UITableView, moveRowAt sourceIndexPath: IndexPath, to destinationIndexPath: IndexPath) {
        print("sourceIndexPath:", sourceIndexPath.section, sourceIndexPath.row, "destinationIndexPath:", destinationIndexPath.section, destinationIndexPath.row)
        let indices:[Int] = getIndicesBySrcAndDest(sourceIndexPath, destinationIndexPath)
        let srcIdx = indices.first!
        let destIdx = indices.last!
        let source = dataArray[srcIdx]
        print("srcIdx:", srcIdx, "destIdx:", destIdx)
        if srcIdx != destIdx{
            dataArray.remove(at: srcIdx)
            dataArray.insert(source, at: destIdx)
        }
